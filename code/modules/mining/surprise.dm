...
/surprise_room/proc/UpdateTurfs()
	for(var/turf/T in turfs)
		UpdateTurf(T)

/surprise_room/proc/GetTurfs(var/ttype)
	var/list/selected[0]
	for(var/turf/T in turfs)
...
/surprise_room/proc/GetTurfInfo(var/turf/T)
...
/surprise_room/proc/UpdateTurf(var/turf/T, var/no_adjacent=0)
...
/surprise_room/proc/AddTypeToTurf(var/turf/T, var/newtype)
	var/surprise_turf_info/sti = GetTurfInfo(T)
	sti.types |= newtype

	//UpdateAdjacentsOfTurf(T)

/surprise_room/proc/UpdateAdjacentsOfTurf(var/turf/T)
...
/surprise_room/proc/IsWall(var/turf/T)
	var/surprise_turf_info/sti = GetTurfInfo(T)
	return sti.turf_type == TURF_WALL

/surprise_room/proc/IsFloor(var/turf/T)
	var/surprise_turf_info/sti = GetTurfInfo(T)
	return sti.turf_type == TURF_FLOOR

	// Are we adjacent to an object of this type?
/surprise_room/proc/AdjacentToType(var/turf/T,var/adjacent_type)
	var/surprise_turf_info/sti = GetTurfInfo(T)
	return locate(adjacent_type) in sti.GetAdjacentTypes()

	// Same, but for walls/floors
/surprise_room/proc/AdjacentToTurfType(var/turf/T,var/turfType)
	for(var/dir in cardinal)
		var/turf/AT = get_step(T,dir)
...
/layout_rule/proc/GetCandidates()
	return list()
...
/layout_rule/place_adjacent/proc/IsTurfCandidate(var/turf/T,var/list/opt_nt,var/list/opt_nnt)
...
/mining_surprise/proc/spawn_complex(var/atom/start_loc)
...
/mining_surprise/proc/postProcessComplex()
...
/mining_surprise/proc/spawn_room(var/atom/start_loc, var/x_size, var/y_size, var/clean = 0)
